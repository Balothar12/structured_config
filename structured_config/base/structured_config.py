...
from structured_config.io.case_translation.case_translator_base import (
    CaseTranslatorBase,
)
...
from structured_config.io.overrides.mapper_builder import (
    MapperBuilder,
...
class ArgparseConfig:
    ...
    config_file: ConfigArgument or None = field(
        default_factory=lambda: ConfigArgument(
            name="config",
            short_name="c",
            required=True,
            positional=True,
    ...
    overrides_list: OverrideListArgument or None = field(
        default_factory=lambda: OverrideListArgument(
            name="override",
            short_name=None,
            required=False,
            default=[],
    ...
    def override(
        self, override_config: "OverrideConfig", arguments: argparse.Namespace
    ):
        if self.use_for_overrides:
            override_config.mapper.build().from_argparse_keylist(
                arguments=arguments, list_name=self.overrides_list.get_destination()
            ).all().apply().apply()
...
class OverrideConfig:
    ...
    mapper: Mapper = field(default_factory=lambda: Mapper())
...
class FileConfig:
    ...
    reader_extension_map: Dict[str, ConfigReaderBase] = field(
    ...
    extension_source_case: Dict[str, CaseTranslatorBase] = field(
        default_factory=lambda: {
            ".json": SnakeCase(),
            ".yaml": PascalCase(),
        }
...
class ConfigSpecification:
    ...
    def _prepare_config(self) -> ConfigObjectType:
        ...
        file: str = None
        if self.arg_config:
            arguments: argparse.Namespace = self.arg_config.setup(
                config=self.specification
            )
            self.arg_config.override(
                override_config=self.override_config, arguments=arguments
            )

            # read data from argparse if possible
            file = getattr(
                arguments,
                self.arg_config.config_file.get_destination()
                if self.arg_config.config_file
                else "_NOT_AN_ATTRIBUTE_",
        ...
        return self.override_config.mapper.with_source_case(
            source_case=self.specification.get_source_case()
        ).apply(to=data)

    def _validate_config(self):
        # check config file location
        if self.file_config.file == None and (
            self.arg_config == None or self.arg_config.config_file == None
        ):
            raise InvalidConfigSpecificationException(
                reason="No config file provided, and file cannot be read from argparse"
            )

        # overrides_list_name must be specified in argparse if it's meant to be used for overrides
        if (
            self.arg_config != None
            and self.arg_config.use_for_overrides
            and self.arg_config.overrides_list == None
        ):
            raise InvalidConfigSpecificationException(
                reason="Override list name must be specified if it should be used"
            )
