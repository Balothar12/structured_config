...
from structured_config.type_checking.type_config import (
...
from structured_config.base.typedefs import (
...
class RequireConfigType:
    ...
    def object() -> ConfigTypeCheckingFunction:
        return ConfigTypeChecker(
            allow_instance_of=False, specific_types=[Dict[str, Any]]
        )
    ...
    @staticmethod
    def from_type_list(
        types: List[Type], allow_instance_of: bool = False
    ) -> ConfigTypeCheckingFunction:
        return ConfigTypeChecker(
            allow_instance_of=allow_instance_of, specific_types=types
        )

    @staticmethod
    def make_type_checking_function(
        expected_types: ScalarConfigTypeRequirements,
        default: ConfigTypeCheckingFunction,
    ) -> ConfigTypeCheckingFunction:
...
class RequireConvertedType:
    ...
    def from_type_list(
        ...
        return ConvertedTypeChecker(allow_instance_of=allow_instance_of, any_of=types)

    @staticmethod
    def make_type_checking_function(
        expected_types: ScalarConvertedTypeRequirements,
        default: ConvertedTypeCheckingFunction,
    ) -> ConvertedTypeCheckingFunction:
        if type(expected_types) is list:
            return RequireConvertedType.from_type_list(types=expected_types)
        elif callable(
            expected_types
        ) and RequireConvertedType._is_type_checking_function(expected_types):
            return expected_types
        elif expected_types:
            return RequireConvertedType.from_type_list(types=[expected_types])
        else:
    ...
    def _is_type_checking_function(type_object: Callable) -> bool:
        ...
        if (
            "key" not in signature.parameters
            or "parent_key" not in signature.parameters
            or "obj" not in signature.parameters
        ):
            return False
